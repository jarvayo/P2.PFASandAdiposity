...
summary(PFAS_BF_NA$mom_age_days_comp_d_ma17)
...
PFAS_BF_NA$marital_epi_epia_d <- as.factor(PFAS_BF_NA$marital_epi_epia_d)

summary(PFAS_BF_NA$race2_mom_epi_epia_d)
...
summary(PFAS_BF_NA$marital_epi_epia_d)    # 2 NAs

##### TABLE OF MISSINGNESS
table1(~ age_mom_enroll_d + mom_age_days_comp_d_ma17 + bmi_mom_prepreg_d + parity_d, 
       data = PFAS_BF_NA, overall="Total")

table1(~ race2_mom_epi_epia_d + coll_grad + income_hh_epq_epqa_d + gt70k +
         smokpreg_final_d + marital_epi_epia_d,
       data = PFAS_BF_NA, overall="Total")
...
summary(PFAS_DXABF_NA$mom_age_days_mdxa17)
summary(PFAS_DXABF_NA$bmi_mom_prepreg_d)     
summary(PFAS_DXABF_NA$parity_d)              
summary(PFAS_DXABF_NA$mod_pre_d)             # 50 NAs
summary(PFAS_DXABF_NA$vig_pre_d)             # 46 NAs
#summary(PFAS_DXABF_NA$income_hh_epq_epqa_d)  # 34 NAs
...
#summary(PFAS_DXABF_NA$parity_d)              
#summary(PFAS_DXABF_NA$income_hh_epq_epqa_d)  # 34 NAs
summary(PFAS_DXABF_NA$gt70k)                 # 34 NAs
summary(PFAS_DXABF_NA$coll_grad)             
summary(PFAS_DXABF_NA$race2_mom_epi_epia_d)
summary(PFAS_DXABF_NA$smokpreg_final_d)      # 1 NAs

##### TABLE OF MISSINGNESS
table1(~ age_mom_enroll_d + mom_age_days_mdxa17 + bmi_mom_prepreg_d + parity_d +
         mod_pre_d + vig_pre_d, data = PFAS_DXABF_NA, overall="Total")

table1(~ race2_mom_epi_epia_d + coll_grad + smokpreg_final_d + gt70k,
...
PFAS_BF_ARG <- aregImpute(~ mod_pre_d + vig_pre_d + alc_d_bef_learn_f1 +
...
PFAS_DXABF_ARG <- aregImpute(~ mod_pre_d + vig_pre_d + alc_d_bef_learn_f1 +
                         alc_d_aft_learn_f1 + alc_d_f1 + income_hh_epq_epqa_d +
                         gt70k + smokpreg_final_d + bmi_mom_prepreg_d + parity_d + 
                         coll_grad + race2_mom_epi_epia_d + g_age_days_COLL_D_BLD1 + 
                         mom_age_days_mdxa17 + PFNA2 + PFHxS + 
...
table1(~ age_mom_enroll_d + mom_age_days_mdxa17 + bmi_mom_prepreg_d + pari y_d +
         mod_pre_d + vig_pre_d, data = PFAS_BF_COMP, overall="Total")
...
##### RE-CHECK VARIABLE DISTRIBUTION FOR DATASET WITH IMPUTED DATA 
table1(~ age_mom_enroll_d + mom_age_days_mdxa17 + bmi_mom_prepreg_d + pari y_d +
         mod_pre_d + vig_pre_d, data = PFAS_DXABF_COMP, overall="Total")
