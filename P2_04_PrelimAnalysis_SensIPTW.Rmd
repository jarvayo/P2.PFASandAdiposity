...
PFAS_IPW1$anthro <- ifelse(is.na(PFAS_W1$LNBMI), 1, 0)
PFAS_IPW1$dxa <- ifelse(is.na(PFAS_W2$LNTKNFAT), 1, 0)

table(PFAS_IPW1$anthro)
table(PFAS_IPW1$dxa)
...
# PFOS
a_n_PFOS <- glm(anthro ~ L2PFOS, family = binomial(), data = PFAS_IPW1)
a_pn_PFOS <- 1-predict(a_n_PFOS, type = "response")

# PFOA
a_n_PFOA <- glm(anthro ~ L2PFOA, family = binomial(), data = PFAS_IPW1)
a_pn_PFOA <- 1-predict(a_n_PFOA, type = "response")

# PFNA
a_n_PFNA <- glm(anthro ~ L2PFNA, family = binomial(), data = PFAS_IPW1)
a_pn_PFNA <- 1-predict(a_n_PFNA, type = "response")

# PFHxS
a_n_PFHxS <- glm(anthro ~ L2PFHxS, family = binomial(), data = PFAS_IPW1)
a_pn_PFHxS <- 1-predict(a_n_PFHxS, type = "response")

# MeFOSAA
a_n_MeFOSAA <- glm(anthro ~ L2MeFOSAA, family = binomial(), data = PFAS_IPW1)
a_pn_MeFOSAA <- 1-predict(a_n_MeFOSAA, type = "response")

# EtFOSAA
a_n_EtFOSAA <- glm(anthro ~ L2EtFOSAA, family = binomial(), data = PFAS_IPW1)
a_pn_EtFOSAA <- 1-predict(a_n_EtFOSAA, type = "response")


##### ESTIMATE THE DENOMINATOR OF CENSORING WEIGHTS FOR ANTHRO DATASET
# PFOS
a_d_PFOS <- glm(anthro ~ L2PFOS + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW1)
a_pd_PFOS <- 1-predict(a_d_PFOS, type = "response")

# PFOA
a_d_PFOA <- glm(anthro ~ L2PFOA + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW1)
a_pd_PFOA <- 1-predict(a_d_PFOA, type = "response")

# PFNA
a_d_PFNA <- glm(anthro ~ L2PFNA + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW1)
a_pd_PFNA <- 1-predict(a_d_PFNA, type = "response")

# PFHxS
a_d_PFHxS <- glm(anthro ~ L2PFHxS + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW1)
a_pd_PFHxS <- 1-predict(a_d_PFHxS, type = "response")

# MeFOSAA
a_d_MeFOSAA <- glm(anthro ~ L2MeFOSAA + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW1)
a_pd_MeFOSAA <- 1-predict(a_d_MeFOSAA, type = "response")

# EtFOSAA
a_d_EtFOSAA <- glm(anthro ~ L2EtFOSAA + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW1)
...
# PFOS
d_n_PFOS <- glm(dxa ~ L2PFOS, family = binomial(), data = PFAS_IPW2)
d_pn_PFOS <- 1-predict(d_n_PFOS, type = "response")

# PFOA
d_n_PFOA <- glm(dxa ~ L2PFOA, family = binomial(), data = PFAS_IPW2)
d_pn_PFOA <- 1-predict(d_n_PFOA, type = "response")

# PFNA
d_n_PFNA <- glm(dxa ~ L2PFNA, family = binomial(), data = PFAS_IPW2)
d_pn_PFNA <- 1-predict(d_n_PFNA, type = "response")

# PFHxS
d_n_PFHxS <- glm(dxa ~ L2PFHxS, family = binomial(), data = PFAS_IPW2)
d_pn_PFHxS <- 1-predict(d_n_PFHxS, type = "response")

# MeFOSAA
d_n_MeFOSAA <- glm(dxa ~ L2MeFOSAA, family = binomial(), data = PFAS_IPW2)
d_pn_MeFOSAA <- 1-predict(d_n_MeFOSAA, type = "response")

# EtFOSAA
d_n_EtFOSAA <- glm(dxa ~ L2EtFOSAA, family = binomial(), data = PFAS_IPW2)
d_pn_EtFOSAA <- 1-predict(d_n_EtFOSAA, type = "response")


##### ESTIMATE THE DENOMINATOR OF CENSORING WEIGHTS FOR DXA DATASET
# PFOS
d_d_PFOS <- glm(dxa ~ L2PFOS + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW2)
d_pd_PFOS <- 1-predict(d_d_PFOS, type = "response")

# PFOA
d_d_PFOA <- glm(dxa ~ L2PFOA + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW2)
d_pd_PFOA <- 1-predict(d_d_PFOA, type = "response")

# PFNA
d_d_PFNA <- glm(dxa ~ L2PFNA + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW2)
d_pd_PFNA <- 1-predict(d_d_PFNA, type = "response")

# PFHxS
d_d_PFHxS <- glm(dxa ~ L2PFHxS + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW2)
d_pd_PFHxS <- 1-predict(d_d_PFHxS, type = "response")

# MeFOSAA
d_d_MeFOSAA <- glm(dxa ~ L2MeFOSAA + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW2)
d_pd_MeFOSAA <- 1-predict(d_d_MeFOSAA, type = "response")

# EtFOSAA
d_d_EtFOSAA <- glm(dxa ~ L2EtFOSAA + age_enroll + race + income + menar + smoke +  
                 parity + married, family = binomial(), data = PFAS_IPW2)
