...
cov_d <- as.matrix(cbind(PFAS_D$age_enroll, PFAS_D$menar, PFAS_D$race, PFAS_D$income, 
                         PFAS_D$parity, PFAS_D$smoke, PFAS_D$married))
...
l2m_trnkfat <- kmbayes(y = LNTRNKFAT, Z = l2mixture_d_z, X = cov_d, iter=25000, family = "gaussian",
...
#                      verbose = FALSE, varsel = TRUE, knots = knots_dln)
l2m_totfat <- kmbayes(y = LNTOTFAT, Z = l2mixture_d_z, X = cov_d, iter=25000, family = "gaussian",
                     verbose = FALSE, varsel = TRUE, knots = knots_dl2_z)
beep(sound=5, expr=NULL)
...
#                      verbose = FALSE, varsel = TRUE, knots = knots_dln)
l2m_totfatpct <- kmbayes(y = TotFatPct, Z = l2mixture_d_z, X = cov_d, iter=25000, family = "gaussian",
                     verbose = FALSE, varsel = TRUE, knots = knots_dl2_z)
beep(sound=5, expr=NULL)

# TRUNK FAT INDEX
l2m_tfin <- kmbayes(y = LNTFIN, Z = l2mixture_d_z, X = cov_d, iter=25000, family = "gaussian",
                     verbose = FALSE, varsel = TRUE, knots = knots_dl2_z)
beep(sound=5, expr=NULL)

# TOTAL BODY FAT INDEX
l2m_bfin <- kmbayes(y = LNBFIN, Z = l2mixture_d_z, X = cov_d, iter=25000, family = "gaussian",
                     verbose = FALSE, varsel = TRUE, knots = knots_dl2_z)
beep(sound=5, expr=NULL)
...
save(l2m_trnkfat, l2m_totfat, l2m_totfatpct, l2m_tfin, l2m_bfin,
...
uni_trnkfat <- ggplot(univar_trnkfat, aes(z, est, ymin = est - 1.96*se, ymax = est + 1.96*se)) + facet_wrap(~ variable) +
  geom_smooth(stat = "identity", colour='black') + xlab("Plasma PFAS, log2(ng/mL)") + ylab("Trunk fat mass (% diff.)")+
...
uni1_trnkfat <- ggplot(univar_trnkfat[1:50,], aes(z, est, ymin = est - 1.96*se, ymax = est + 1.96*se)) +
  ...
  xlab("Plasma PFOS, log2(ng/mL)") + ylab("Trunk fat mass (% diff.)")+
...
p_overall_trnkfat <- ggplot(overall_trnkfat, aes(quantile, est, ymin = est - 1.96*sd, ymax = est + 1.96*sd)) +
               ...
               geom_pointrange() +
               scale_y_continuous(name = "Trunk fat mass (% diff.)")+
               xlab("PFAS quantile") +
               theme_classic()+ theme(
...
p_overall_totfat <- ggplot(overall_totfat, aes(quantile, est, ymin = est - 1.96*sd, ymax = est + 1.96*sd)) +
               geom_hline(yintercept = 00, linetype = "dashed", linewidth = 1, color = "red") +
               geom_pointrange() +
               scale_y_continuous(name = "Body fat mass (% diff.)")+
               xlab("PFAS quantile") +
               theme_classic()+ theme(
               axis.text.x = element_text(size = 25),
               axis.text.y = element_text(size = 25),
