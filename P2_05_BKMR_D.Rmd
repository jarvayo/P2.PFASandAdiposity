...
##### LOAD WORKSPACE
# load("C:/Users/jarva/Desktop/James-Todd Lab/PFASandAdiposity/Data/....Rdata")
...
cov_d <- as.matrix(cbind(PFAS_D$age_enroll, PFAS_D$race, PFAS_D$menar, PFAS_D$edu, 
...
# save(lnzm_trnkfat, lnzm_totfat, lnz_totfatpct,
#     file="C:/Users/jarva/Desktop/James-Todd Lab/PFASandAdiposity/Data/P3_BKMR_D_LNZ.Rdata")
# load("C:/Users/jarva/Desktop/James-Todd Lab/PFASandAdiposity/Data/P3_BKMR_D_LNZ.Rdata")

# save(lnm_trnkfat, lnm_totfat, lz_totfatpct,
#      file="C:/Users/jarva/Desktop/James-Todd Lab/PFASandAdiposity/Data/P3_BKMR__LN.Rdata")
# load("C:/Users/jarva/Desktop/James-Todd Lab/PFASandAdiposity/Data/P3_BKMR_D_LN.Rdata")

save(l2zm_trnkfat, l2zm_totfat, l2z_totfatpct,
...
TracePlot(fit = lnzm_trnkfat, par = "beta", sel = sel)
TracePlot(fit = lnzm_totfat, par = "beta", sel = sel)
TracePlot(fit = lnzm_totfatpct, par = "beta", sel = sel)
...
# TOTAL TRUNK FAT MASS
#summary(m_trnkfat)
ExtractPIPs(lnzm_trnkfat, sel = sel_final)

# TOTAL BODY FAT MASS
#summary(m_totfat)
ExtractPIPs(lnzm_totfat, sel = sel_final)

# TOTAL BODY FAT PERCENT
#summary(m_totfatpct)
ExtractPIPs(lnzm_totfatpct, sel = sel_final)
...
# TOTAL TRUNK FAT MASS
univar_trnkfat <- PredictorResponseUnivar(fit = lnzm_trnkfat, sel = sel_final, method = "approx")
univar_trnkfat

# TOTAL BODY FAT MASS
univar_totfat <- PredictorResponseUnivar(fit = lnzm_totfat, sel = sel_final, method = "approx")
univar_totfat

# TOTAL BODY FAT PERCENT
univar_totfatpct <- PredictorResponseUnivar(fit = lnzm_totfatpct, sel = sel_final, method = "approx")
univar_totfatpct
...
uni_trnkfat <- ggplot(univar_trnkfat, aes(z, est, ymin = est - 1.96*se, ymax = est + 1.96*se)) + facet_wrap(~ variable) +
  geom_smooth(stat = "identity", colour='black') + xlab("Plasma PFAS, ln(ng/mL)") + ylab("Trunk fat mass (kg)")+
...
uni1_trnkfat <- ggplot(univar_trnkfat[1:50,], aes(z, est, ymin = est - 1.96*se, ymax = est + 1.96*se)) +
  ...
  xlab("Plasma PFOS, ln(ng/mL)") + ylab("Trunk fat mass (kg)")+
...
# TOTAL TRUNK FAT MASS
bivar_trnkfat <- PredictorResponseBivar(fit = lnzm_trnkfat, min.plot.dist = 1, sel = sel_final, method = "approx")
interact_trnkfat <- PredictorResponseBivarLevels(pred.resp.df = bivar_trnkfat, Z = lnmixture_d_z,
                                               both_pairs = T, qs = c(0.25, 0.50, 0.75))
interact_trnkfat_1090 <- PredictorResponseBivarLevels(pred.resp.df = bivar_trnkfat, Z = lnmixture_d_z,
                                                    both_pairs = TRUE, qs = c(0.10, 0.5, 0.90))

# TOTAL BODY FAT MASS
bivar_totfat <- PredictorResponseBivar(fit = lnzm_totfat, min.plot.dist = 1, sel = sel_final, method = "approx")
interact_totfat <- PredictorResponseBivarLevels(pred.resp.df = bivar_totfat, Z = lnmixture_d_z,
                                               both_pairs = T, qs = c(0.25, 0.50, 0.75))
interact_totfat_1090 <- PredictorResponseBivarLevels(pred.resp.df = bivar_totfat, Z = lnmixture_d_z,
                                                    both_pairs = TRUE, qs = c(0.10, 0.5, 0.90))

# TOTAL BODY FAT PERCENT
bivar_totfatpct <- PredictorResponseBivar(fit = lnzm_totfatpct, min.plot.dist = 1, sel = sel_final, method = "approx")
interact_totfatpct <- PredictorResponseBivarLevels(pred.resp.df = bivar_totfatpct, Z = lnmixture_d_z,
                                               both_pairs = T, qs = c(0.25, 0.50, 0.75))
interact_totfatpct_1090 <- PredictorResponseBivarLevels(pred.resp.df = bivar_totfatpct, Z = lnmixture_d_z,
                                                    both_pairs = TRUE, qs = c(0.10, 0.5, 0.90))

# # TOTAL BODY LEAN MASS
# bivar_totlean <- PredictorResponseBivar(fit = m_totlean, min.plot.dist = 1, sel = sel_final, method = "approx")
# interact_totlean <- PredictorResponseBivarLevels(pred.resp.df = bivar_totlean, Z = lnmixture_d_z,
#                                                both_pairs = T, qs = c(0.25, 0.50, 0.75))
# interact_totlean_1090 <- PredictorResponseBivarLevels(pred.resp.df = bivar_totlean, Z = lnmixture_d_z,
#                                                     both_pairs = TRUE, qs = c(0.10, 0.5, 0.90))
...
# TOTAL TRUNK FAT MASS
overall_trnkfat <- OverallRiskSummaries(fit = lnzm_trnkfat, q.fixed = 0.50, sel = sel_final,
                                               method = "approx", qs = seq(0.25, 0.75, by = 0.05))

# TOTAL BODY FAT MASS
overall_totfat <- OverallRiskSummaries(fit = lnzm_totfat, q.fixed = 0.50, sel = sel_final,
                                               method = "approx", qs = seq(0.25, 0.75, by = 0.05))

# TOTAL BODY FAT PERCENT
overall_totfatpct <- OverallRiskSummaries(fit = lnzm_totfatpct, q.fixed = 0.50, sel = sel_final,
                                               method = "approx", qs = seq(0.25, 0.75, by = 0.05))
...
# TOTAL TRUNK FAT MASS
singvar_trnkfat <- SingVarRiskSummaries(fit = lnzm_trnkfat, qs.diff = c(0.25, 0.75),
                                            q.fixed = c(0.25, 0.50, 0.75),sel=sel_final, method = "approx")

# TOTAL BODY FAT MASS
singvar_totfat <- SingVarRiskSummaries(fit = lnzm_totfat, qs.diff = c(0.25, 0.75),
                                            q.fixed = c(0.25, 0.50, 0.75),sel=sel_final, method = "approx")

# TOTAL BODY FAT PERCENT
singvar_totfatpct <- SingVarRiskSummaries(fit = lnzm_totfatpct, qs.diff = c(0.25, 0.75),
                                            q.fixed = c(0.25, 0.50, 0.75),sel=sel_final, method = "approx")
...
p_singvar_trnkfat <- ggplot(singvar_trnkfat, aes(variable, est, ymin = est - 1.96*sd, ymax = est + 1.96*sd,
  ...
  scale_x_discrete(name = "Plasma PFAS, ln(ng/mL)") + scale_y_continuous(name = "Trunk fat mass (kg)")
p_singvar_trnkfat
...
#   geom_hline(aes(yintercept = 0), linetype = "dashed", linewidth = 1, color = "black") +
#   geom_pointrange(position = position_dodge(width = 0.75)) + coord_flip() +
#   scale_x_discrete(name = "Plasma PFAS, ln(ng/mL)") + scale_y_continuous(name = "Body lean mass (kg)")
# p_singvar_totlean
...
# TOTAL TRUNK FAT MASS
overall_interact_trnkfat <- SingVarIntSummaries(fit = lnzm_trnkfat, qs.diff = c(0.25, 0.75),
                                       qs.fixed = c(0.25, 0.75),sel=sel_final)

# TOTAL BODY FAT MASS
overall_interact_totfat <- SingVarIntSummaries(fit = lnzm_totfat, qs.diff = c(0.25, 0.75),
                                       qs.fixed = c(0.25, 0.75),sel=sel_final)

# TOTAL BODY FAT PERCENT
overall_interact_totfatpct <- SingVarIntSummaries(fit = lnzm_totfatpct, qs.diff = c(0.25, 0.75),
                                       qs.fixed = c(0.25, 0.75),sel=sel_final)
